...
TEST(CreateLineStartsTest, SingleLF) {
  ...
  std::vector<size_t> expected = {
      0,
      6};  // Line1 starts at 0, Line2 starts after '\n' at index 5 -> 5+1 = 6
  EXPECT_EQ(createLineStarts(str), expected);
}
...
TEST(CreateLineStartsTest, SingleCRLF) {
  auto str = toWCharVector("Line1\r\nLine2");
  std::vector<size_t> expected = {
      0, 7};  // Line1 @0, Line2 starts after '\r\n' at index 5+2 = 7
  EXPECT_EQ(createLineStarts(str), expected);
}
...
TEST(CreateLineStartsTest, MultipleCRLFs) {
  ...
  std::vector<size_t> expected = {
      0, 7, 15, 22};  // 'F' @0, 'S' @7, 'T' @15, empty line @22
  EXPECT_EQ(createLineStarts(str), expected);
}
...
TEST_F(PieceTreeTest, EmptyInputStringConstructor) {
  ...
  EXPECT_EQ(pt.buffers.size(),
            2);  // Initial empty buffer + the empty input buffer
}
...
TEST_F(PieceTreeTest, NonEmptyInputStringConstructor) {
  ...
  ASSERT_EQ(pt.buffers.size(),
            2);  // Initial empty buffer (index 0) + input buffer (index 1)
  EXPECT_EQ(pt.buffers[1].value,
            input);  // Ensure input buffer is correctly stored
  ...
  // For end position: lineStarts.size() - 1 for index, initialPiece.size() -
  // lineStarts.back() for offset "Hello\nWorld" -> createLineStarts returns {0,
  // 6}. Size is 2. lineStarts.size() - 1 = 1 lineStarts.back() = 6
  // initialPiece.size() = 11
  EXPECT_EQ(initialPiece.end.line,
            1);  // Index of the last line in piece (the second line, index 1)
  EXPECT_EQ(initialPiece.end.column,
            5);  // Offset of the start of the last line (the 'W' in World)
}
...
TEST_F(PieceTreeTest, InsertInMiddle_SplitsPiece) {
  ...
  auto textToInsert =
      toWCharVector(" ");  // Insert space at offset 5 ("Hello_World")
  ...
  // The original piece ("HelloWorld") should be split into two pieces ("Hello"
  // and "World"), and a new piece (" ") should be inserted between them.
  EXPECT_EQ((pt.GetText(0, 99)), toWCharVector("Hello World"));
}
...
TEST_F(PieceTreeTest, GetText_LengthExceedsDocument) {
  ...
  EXPECT_EQ(toWCharVector("Test"),
            pt->GetText(0, 10));  // Should return full "Test"
...
TEST_F(PieceTreeTest, GetText_AcrossMultiplePieces) {
  ...
  pt->InsertText(9,
                 toWCharVector("MiddlePart"));  // "FirstPartMiddlePartLastPart"
