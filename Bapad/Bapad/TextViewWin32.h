...
auto inline TextView_SetLineSpacing(HWND hwndTV, int nAbove,
                                    int nBelow) noexcept {
  return SendMessageW((hwndTV), TXM_SETLINESPACING, nAbove, nBelow);
}

auto inline TextView_AddFont(HWND hwndTV, HFONT hFont,
                             HWND g_hwndTextView) noexcept {
  return SendMessageW((hwndTV), TXM_ADDFONT, (WPARAM)(HFONT)(g_hwndTextView),
                      0);
}

auto inline TextView_SetColor(HWND hwndTV, WPARAM nIdx,
                              LPARAM rgbColor) noexcept {
  return SendMessageW((hwndTV), TXM_SETCOLOR, nIdx, rgbColor);
}

auto inline TextView_SetStyle(HWND hwndTV, WPARAM uMask,
                              LPARAM uStyles) noexcept {
  return SendMessageW((hwndTV), TXM_SETSTYLE, uMask, uStyles);
}

auto inline TextView_SetStyleBool(HWND hwndTV, WPARAM uStyle,
                                  LPARAM fBoolean) noexcept {
...
auto inline TextView_SetImageList(HWND hwndTV, HIMAGELIST hImgList) noexcept {
  return SendMessageW((hwndTV), TXM_SETIMAGELIST,
                      (WPARAM)(HIMAGELIST)(hImgList), 0);
}

auto inline TextView_SetLongLine(HWND hwndTV, LPARAM nLength) noexcept {
  return SendMessageW((hwndTV), TXM_SETLONGLINE, (WPARAM)(0),
                      (LPARAM)(nLength));
}

auto inline TextView_SetLineImage(HWND hwndTV, ULONG nLineNo,
                                  ULONG nImageIdx) noexcept {
  return SendMessageW((hwndTV), TXM_SETLINEIMAGE, (WPARAM)(ULONG)(nLineNo),
