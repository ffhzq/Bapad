...
LONG TextView::OnSize(UINT nFlags, int width, int height) {
  windowLines =
      height / lineHeight;  // min((unsigned)height / lineHeight, lineCount);
...
//	ScrollRgn
//
//	Scrolls the viewport in specified direction. If fReturnUpdateRgn is
//true, 	then a HRGN is returned which holds the client-region that must be
//redrawn 	manually. This region must be deleted by the caller using
//DeleteObject.
//
...
HRGN TextView::ScrollRgn(LONG64 dx, LONG64 dy, bool fReturnUpdateRgn) {
  ...
  // scroll up
  if (dy < 0) {
    dy = -1 * std::min((ULONG64)-dy, vScrollPos);
    clip.top = -dy * lineHeight;
  }
  // scroll down
  else if (dy > 0) {
    dy = std::min((ULONG64)dy, vScrollMax - vScrollPos);
    clip.bottom = (windowLines - dy) * lineHeight;
  }

  // scroll left
  if (dx < 0) {
    dx = -(int)std::min(-dx, hScrollPos);
    clip.left = -dx * fontWidth * 4;
  }
  // scroll right
  else if (dx > 0) {
    dx = std::min(dx, (hScrollMax - hScrollPos));
...
VOID TextView::Scroll(LONG64 dx, LONG64 dy) { ScrollRgn(dx, dy, false); }
...
#define SPI_GETWHEELSCROLLLINES 104
  ...
  if (nScrollLines <= 1) nScrollLines = 3;
