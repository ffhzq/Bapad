...
void TextView::MoveCharNext() {
  ...
  TextIterator itor = pTextDoc->IterateLineByLineNumber(
      currentLine, &linestartCharOffset, &lineLenghCharOffset);
  ...
  if (index + 1 > lineContent.size()) {
    ...
    if (currentLine + 1 < lineCount &&
        lineLenghCharOffset - 2 ==
            cursorOffset)  // if hit CRLF, then go next line
    {
      pTextDoc->IterateLineByLineNumber(currentLine + 1, &cursorOffset,
                                        nullptr);
    } else
      cursorOffset += 1;
  }
...
void TextView::MoveCharPrev() {
  size_t lineNumber = 0;
  size_t linestartCharOffset = 0;
  TextIterator itor = pTextDoc->IterateLineByCharOffset(
      cursorOffset, &lineNumber, &linestartCharOffset);
  auto lineContent = itor.GetLine();
  const int index = cursorOffset - linestartCharOffset;
...
void TextView::MoveLineUp(int numOfLines) {
  ...
  auto originalItor = pTextDoc->IterateLineByLineNumber(
      currentLine, &linestartCharOffset, nullptr);
  const int lineIndexOffset = cursorOffset - linestartCharOffset;
  if (currentLine < numOfLines) numOfLines = currentLine;

  auto itor = pTextDoc->IterateLineByLineNumber(currentLine - numOfLines,
                                                &newLinestartCharOffset,
                                                &newLineLenghCharOffset);
  ...
  if (currentLine - numOfLines != lineCount - 1 &&
      lineIndexOffset + 2 > newLineLenghCharOffset) {
...
void TextView::MoveLineDown(int numOfLines) {
  ...
  auto originalItor = pTextDoc->IterateLineByLineNumber(
      currentLine, &linestartCharOffset, nullptr);
  const int lineIndexOffset = cursorOffset - linestartCharOffset;
  if (currentLine + numOfLines >= n) numOfLines = n - currentLine - 1;
  auto itor = pTextDoc->IterateLineByLineNumber(currentLine + numOfLines,
                                                &newLinestartCharOffset,
                                                &newLineLenghCharOffset);
  if (newLineLenghCharOffset > lineIndexOffset)
...
void TextView::MoveLineStart(int lineNumber) {
  size_t newLinestartCharOffset = 0;
  auto itor = pTextDoc->IterateLineByLineNumber(
      lineNumber, &newLinestartCharOffset, nullptr);
...
void TextView::MoveLineEnd(int lineNumber) {
  ...
  size_t newLineLenghCharOffset = 0;
  auto itor = pTextDoc->IterateLineByLineNumber(
      lineNumber, &newLinestartCharOffset, &newLineLenghCharOffset);
  cursorOffset = newLinestartCharOffset + newLineLenghCharOffset;
  if (lineCount > lineNumber + 1) cursorOffset -= 2;
  SyncMetrics(TRUE);
}