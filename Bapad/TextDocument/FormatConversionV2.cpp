 ...
 */
  ...
  size_t i = 0;
  while (i < len) {
    const auto ch = static_cast<unsigned char>(bufferSpan[i]);
    if (ch < 0x80) {
      i++;
    } else if ((ch & 0xE0) == 0xC0) {
      if (i + 1 >= len) return false;
      if ((static_cast<unsigned char>(bufferSpan[i + 1]) & 0xC0) != 0x80)
        return false;
      i += 2;
    } else if ((ch & 0xF0) == 0xE0) {
      if (i + 2 >= len) return false;
      if ((static_cast<unsigned char>(bufferSpan[i + 1]) & 0xC0) != 0x80 ||
          (static_cast<unsigned char>(bufferSpan[i + 2]) & 0xC0) != 0x80)
        return false;
      i += 3;
      ...
      if ((static_cast<unsigned char>(bufferSpan[i + 1]) & 0xC0) != 0x80 ||
          (static_cast<unsigned char>(bufferSpan[i + 2]) & 0xC0) != 0x80 ||
          (static_cast<unsigned char>(bufferSpan[i + 3]) & 0xC0) != 0x80)
        return false;
      i += 4;
    } else {
      return false;
