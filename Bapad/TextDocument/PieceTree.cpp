...
PieceTree::PieceTree() noexcept
    : buffers{}, rootNode(), _lastChangeBufferPos(), length(0), lineCount(1) {
  auto input = std::vector<char16_t>();
  Init(input);
}

PieceTree::PieceTree(std::vector<char16_t> input)
    : buffers{}, rootNode(), _lastChangeBufferPos(), length(0), lineCount(1) {
  Init(input);
}
...
bool PieceTree::InsertText(size_t offset, const std::vector<char16_t>& input) {
  ...
  buffer.lineStarts.insert(buffer.lineStarts.end(), lineStarts.begin() + 1,
                           lineStarts.end());
  if (node != rootNode.get()) {
    assert(node);
    if (node &&
        pieceOffset !=
            node->piece.length)  // insert inside a piece, split then insert
    {
      if (pieceOffset == 0)
        node = node->left;
      else
    ...
    if (piece.bufferIndex == 0 && piece.end.line == _lastChangeBufferPos.line &&
        piece.end.column == _lastChangeBufferPos.column &&
        offset == piece.length + node->size_left) {  // append to piece
      // appendToNode(nodePos, input);
      const size_t endIndex = buffer.lineStarts.size() - 1,
                   endColumn = buffer.value.size() -
                               gsl::at(buffer.lineStarts, endIndex);
...
bool PieceTree::EraseText(size_t offset, size_t eraseLength) {
  ...
  while (currentNode != nullptr && bytesErased < eraseLength) {
    ...
    const size_t bytesToBeErased =
        (std::min)(currentNode->piece.length, remainBytesToBeErased);
  ...
  if (parrentNode) {
    ...
    if (currentNode == nullptr) {
      ...
      parrentNode->right = currentNode->piece.length == 0
                               ? std::move(currentNode->right)
                               : std::move(currentNode->left->right);
    }
  } else {
    return false;
  }
...
bool PieceTree::ReplaceText(size_t offset, const std::vector<char16_t>& input,
                            size_t erase_length) {
...
NodePosition PieceTree::GetNodePositionAt(TreeNode* node,
                                          size_t offset) noexcept {
  size_t inPieceOffset = offset;
  TreeNode* ptr = node;
  if (ptr == nullptr || inPieceOffset <= ptr->piece.length)
...
TreeNode* PieceTree::SplitPiece(TreeNode* currNode,
  ...
  const size_t newStartOffset =
      offsetInBuffer(currNode->piece.bufferIndex, current_piece.start) +
      inPieceOffset;
  const size_t newLineIndex = GetLineIndexFromNodePosistion(
      currBuffer.lineStarts, NodePosition(currNode, inPieceOffset));
  current_piece.end = BufferPosition{
      newLineIndex,
      newStartOffset - gsl::at(currBuffer.lineStarts, newLineIndex)};
  ...
  if (inPieceOffset == 0) {
    ...
    p2 =
        Piece{current_piece.end, original_piece.end, original_piece.bufferIndex,
...
std::vector<char16_t> PieceTree::GetText(size_t offset, size_t text_length) {
  return GetTextAt(this->rootNode->right.get(), offset, text_length);
}

std::vector<char16_t> PieceTree::GetTextAt(TreeNode* node, size_t offset,
                                           size_t text_length) {
  auto text = std::vector<char16_t>();
  ...
  while (current_node != nullptr && bytes_copied < text_length) {
    ...
    in_piece_offset_start =
        (bytes_copied == 0 ? in_piece_offset_start : 0);  // first piece or not
    ...
    const size_t available_bytes_in_piece =
        piece.length - in_piece_offset_start;
    const size_t bytes_to_be_copied =
        (std::min)(available_bytes_in_piece, remain_bytes_to_be_copied);
...
std::vector<char16_t> PieceTree::GetLine(size_t lineNumber,
                                         const size_t endOffset,
                                         size_t* retValStartOffset) const {
  if (lineNumber > lineCount || lineCount == 0 || lineNumber == 0)
  ...
  auto retVal = std::vector<char16_t>();
  while (node) {
    if (node->piece.lineFeedCnt >
        lineNumber - 1)  // lineContent within this piece
    {
      const size_t preAccValue =
          lineNumber < 2 ? 0 : getAccumulatedValue(node, lineNumber - 2);
      const size_t AccValue = getAccumulatedValue(node, lineNumber - 1);
      const Buffer& buffer = gsl::at(buffers, node->piece.bufferIndex);
      const size_t startOffset =
          offsetInBuffer(node->piece.bufferIndex, node->piece.start);
      ...
      return std::vector<char16_t>(beginItor + preAccValue,
                                   beginItor + AccValue - endOffset);
    } else if (node->piece.lineFeedCnt ==
               lineNumber - 1)  // lineContent in the last line of this piece.
    {
      const size_t preAccValue =
          lineNumber < 2 ? 0 : getAccumulatedValue(node, lineNumber - 2);
      const Buffer& buffer = gsl::at(buffers, node->piece.bufferIndex);
      ...
      const auto& beginItor = buffer.value.begin() + startOffset;
      if (retValStartOffset) *retValStartOffset = preAccValue + node->size_left;
      retVal = std::vector<char16_t>(beginItor + preAccValue,
                                     beginItor + node->piece.length);
      break;
    } else {
  ...
  while (node) {
    ...
    if (piece.lineFeedCnt >
        0)  // the last few chars in fisrt line of this piece.
    {
...
void PieceTree::ShrinkPiece(TreeNode* current_node, size_t shrink_to_right,
  ...
  const size_t offset1 = offsetInBuffer(piece.bufferIndex, piece.start) +
                         shrink_to_right,
               offset2 = offsetInBuffer(piece.bufferIndex, piece.end) -
                         shrink_to_left;

  if (offset1 <= offset2) {
    const size_t lineIndex1 = GetLineIndexFromNodePosistion(
                     lineStarts, NodePosition(current_node, shrink_to_right)),
                 lineIndex2 = GetLineIndexFromNodePosistion(
                     lineStarts,
                     NodePosition(current_node, piece.length - shrink_to_left));
...
size_t PieceTree::getAccumulatedValue(const TreeNode* node,
                                      size_t index) const {
  if (index < 0) return 0;
  const Piece& piece = node->piece;
...
size_t PieceTree::getLongestLine() const {  // todo: brute force
...
std::vector<size_t> createLineStarts(const std::vector<char16_t>& str) {
  ...
  for (size_t i = 0; i < length; ++i) {
    ...
    if (ch == static_cast<char16_t>(CharCode::CarriageReturn)) {
      if (i + 1 < length &&
          gsl::at(str, i + 1) == static_cast<char16_t>(CharCode::LineFeed)) {
        // \r\n
    ...
    } else if (ch == static_cast<char16_t>(CharCode::LineFeed)) {
      // \n
      lineStarts.push_back(i + 1);
    }
  }
...
size_t GetLineIndexFromNodePosistion(const std::vector<size_t>& lineStarts,
  ...
  const size_t startLineOffset =
      gsl::at(lineStarts, nodePos.node->piece.start.line) +
      nodePos.node->piece.start.column;
  const size_t offset = nodePos.in_piece_offset + startLineOffset;
  const size_t startLine = nodePos.node->piece.start.line,
               endLine = nodePos.node->piece.end.line;
  auto it = std::upper_bound(lineStarts.begin() + startLine,
                             lineStarts.begin() + endLine + 1, offset);
...
Buffer::Buffer(std::vector<char16_t> input)
    : value(input), lineStarts(createLineStarts(value)) {}